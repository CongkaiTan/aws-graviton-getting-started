...
1. Check that any startup scripts that attempt to do thread-pinning via `taskset` or `CGroups` are not making assumptions about the presence of SMT that is common on x86 servers.  Graviton processors do not implement SMT (Hyper-Threading), so vCPUs map 1:1 to physical cores and assumptions about two hardware threads per core are incorrect.  This may be present as code taking the number of CPUs in the system and dividing by two, i.e. for shell run scripts you might see: `let physical_cores=$(nproc) / 2`
